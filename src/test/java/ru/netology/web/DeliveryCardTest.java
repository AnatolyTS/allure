...
public class DeliveryCardTest {
    ...
    void shouldSendFormWithCorrectData() {
        ...
        $("[data-test-id=success-notification] .notification__content").shouldHave(text("Встреча успешно " +
                "запланирована на "+DataGenerator.getCorrectDate(3)));
    ...
    void shouldSendFormWithAnotherDateAndTheSameData() {
        ...
        $("[data-test-id=success-notification] .notification__content").shouldHave(text("Встреча успешно " +
                "запланирована на "+DataGenerator.getCorrectDate(7)));
    }
    ...
    void shouldNotSendFormIfNotCorrectCity() {
        ...
        $("[data-test-id='agreement']").click();
        $$("button").find(exactText("Запланировать")).click();
        $("[data-test-id=city].input_invalid .input__sub").shouldHave(exactText("Доставка в выбранный город " +
                "недоступна"));
    }
    ...
    @Test
    void shouldSendFormIfNameWithEngSymbol() {
        $("[data-test-id='city'] input").setValue(DataGenerator.getRandomCity());
        $("[data-test-id='date'] input").setValue(DataGenerator.getCorrectDate(3));
        $("[data-test-id='name'] input").setValue(DataGenerator.getNotCorrectNameWithEngSymbol());
        $("[data-test-id='phone'] input").setValue(DataGenerator.getRandomPhone());
        $("[data-test-id='agreement']").click();
        $$("button").find(exactText("Запланировать")).click();
        $(byText("Успешно!")).waitUntil(visible, 5000);
    ...
    void shouldNotSendFormIfNotAgreement() {
        ...
        $("[data-test-id='phone'] input").setValue(DataGenerator.getRandomPhone());
        $$("button").find(exactText("Запланировать")).click();
        $("[data-test-id=agreement].input_invalid .checkbox__text").shouldHave(exactText("Я соглашаюсь с " +
